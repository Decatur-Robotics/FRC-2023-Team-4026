package frc.robot;

public class Ports {
   public final static int RIGHT_DRIVE_FALCON_FRONT = 2;
   public final static int RIGHT_DRIVE_FALCON_BACK = 4;
   public final static int LEFT_DRIVE_FALCON_FRONT = 3;
   public final static int LEFT_DRIVE_FALCON_BACK = 1;
   public final static int INTAKE_MOTOR = 6;
   public final static int ELEVATOR_MOTOR_MAIN = 18; //Normally 18
   // public final static int ELEVATOR_MOTOR_SUB = 0;
   public final static int PNEUMATICS_HUB = 5;
   public final static int CLAW_CLOSE = 13; //Do these later
   public final static int CLAW_OPEN = 15;
   public final static int ELEVATOR_POTENTIOMETER = 0;
}

